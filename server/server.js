...
app.post("/logincust", (req, res) => {
  ...
  db.query(logincust, [email], (err, result) => {
    if (err) {
      res.send({ message: "Error Occured" });
    ...
    if (result.length > 0) {
      bcrypt.compare(password, result[0].cust_password, (err, response) => {
        if (response) {
          res.send({ message: "Correct", name: result[0].firstname, email: result[0].email});
        } else {
    ...
    // if (response.length > 0) {
    //   if (response[0].cust_password == password) {
    //     res.send({ message: "Correct", name: result[0].firstname, email: result[0].email});
    //   } else {
    //     res.send({
    //       message: "Wrong username/Password Combination",
...
app.post("/updateprofile",(req,res) =>{
  ...
  const query = "UPDATE cust_reg  SET firstname = ?, lastname = ?, email = ?,number = ?, region =?  WHERE email=?";
  db.query(query, [firstname,lastname, email, phonenumber,region,custemail] ,(err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send({ message: result });
...
app.post("/customer/dashboard/solvedtickets", (req, res) => {
  ...
  const status = "solved";
  const query =
    "SELECT * from subitquery_cust WHERE name=? && status=? ";
